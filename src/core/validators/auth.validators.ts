...
import { emailRegex, nameRegex, nickNameRegex, passwordRegex } from '@shared/constants/index';
...
const noInvalidSpaces = (messageLeading: string, messageDouble: string, messageTrailing: string) =>
  z
    .string()
...
export const LoginSchema = z.object({
  ...
  password: z
    ...
    .refine((password) => !/\s/.test(password), strings.passwordCannotContainSpaces),
});
...
export const RegisterSchema = LoginSchema.extend({
  firstName: z
    ...
    .pipe(noInvalidSpaces(strings.fullName_error, strings.nameCannotHaveMultipleSpaces, strings.nameCannotEndWithSpace)),
  lastName: z
    .string()
    .trim()
    ...
    .pipe(noInvalidSpaces(strings.lastName_error, strings.lastNameCannotHaveMultipleSpaces, strings.lastNameCannotEndWithSpace)),
  userName: z
    .string()
    .trim()
    ...
    .refine((val) => !val.trimEnd().endsWith(' '), strings.nickName_cannot_end_space),
  confirmPassword: z
    .string()
    .min(1, strings.enterConfirmPassword)
    .min(6, strings.confirmPasswordMinLength)
    .regex(REGEX.PASSWORD, strings.changeConfirmPasswordText)
    .refine((confirmPassword) => !/\s/.test(confirmPassword), strings.confirmPasswordCannotContainSpaces),
